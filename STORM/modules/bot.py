...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __sᴍᴀᴜɢ>>>__")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🏓 ᴘᴏɴɢ__\n» `{mp} ᴍꜱ`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`ʀᴇsᴛᴀʀᴛɪɴɢ ʙᴏᴛ ᴡᴀɪᴛ ᴀ ᴍɪɴᴜᴛᴇ ᴛʜᴇɴ ᴛʀʏ...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
    ...
    if event.sender_id == OWNER_ID:
        ...
        if str(target) in sudousers:
            ...
            if len(sudousers) > 0:
                ...
                newsudo = f"{target}"
            await ok.edit(f"» **ɴᴇᴡ ꜱᴜᴅᴏ ᴜꜱᴇʀ**: `{target}`\n» `ʀᴇsᴛᴀʀᴛɪɴɢ ʙᴏᴛ ᴡᴀɪᴛ ᴀ ᴍɪɴᴜᴛᴇ ᴛʜᴇɴ ᴛʀʏ...`")
